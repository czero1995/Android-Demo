...
public class WaimaiListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			waimaiHolder.imgShop= (ImageView) convertView.findViewById(R.id.img_shop);
		...
		AQuery aqImg = new AQuery(waimaiHolder.imgShop);
		...
		if (null != shop.getPicShop() &&  !shop.getPicShop().getFileUrl(mContext).isEmpty()) {
			...
			if(url != null && !url.isEmpty()) {
				...
				aqImg.id(R.id.img_shop).image(R.drawable.ic_app);
			}
		}
